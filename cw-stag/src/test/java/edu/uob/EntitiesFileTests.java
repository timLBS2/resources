...
final class EntitiesFileTests {
  ...
  void testBasicEntitiesFileIsReadable() {
      try {
          ...
          parser.parse(reader);
          Graph wholeDocument = parser.getGraphs().get(0);  // There should only be one graph in the file
          ArrayList<Graph> sections = wholeDocument.getSubgraphs(); // There should be two subgraphs in the file

          // The locations will always be in the first subgraph
          ArrayList<Graph> locations = sections.get(0).getSubgraphs();
          Graph firstLocation = locations.get(0);   //subGraph cluster001
          Node locationDetails = firstLocation.getNodes(false).get(0);  // Node cabin
          // Yes, you do need to get the ID twice !
          String locationName = locationDetails.getId().getId();    // cabin
